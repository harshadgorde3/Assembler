...
fp=open("assembly.asm","r")
fp1=open("LST.txt","w")
fp2=open("OBJCODE.txt","w")
def check(synames,sylist,l_no):
	if synames in sylist:
		print 'LINE ',l_no,":Symbol redefined",'"',synames,'"'
		sys.exit()
	elif synames[0] in ck:
		print 'LINE ',l_no,":Symbol name not correct",'"',synames,'"'
		sys.exit()
	else:
		return 1
def hexconvert(sum1):
...
def sympos(sym):
	...
	while(sy_names[i] != sym):
		i = i+1
	return i
def mod_calculatorlit(op1,op2,tp):
	for i in reglsth:
		if op1 in reglsth[i]:
			str1=i
	return str(int(final[0:4],2))+str(int(final[4:8],2))

def calline(f,s,lno,name,sum1,line,temp):
	k =0
	i =0
	z=0 
	if f in reg and s in sy_names:		
		k=sympos(s)
		i=0
		while(reg[i]!=f):
			i=i+1
